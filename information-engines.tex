...
\(D_{\stateVariables|\dataVariables}\), which we refer to as the model
discrimination, 
\[
...
The simulation discrimination is recognised as the Kullback-Leibler
...
We borrow the term discrimination from Kullback himself. He referred to the divergence as representing the average information you get to discriminate between the two distributions per sample from the distribution. By minimizing this discrimination information you ensure that the two distributions become identical. In our case, the distributions are over 'all microstates in the universe', so the discrimination information we are measuring is the information we get to discriminate between the true universe, and the universe implied by our approximation $\physicsProb(\cdot)$.
...
available energy. We might be maximizing simulation discrimination.

\subsection{Available Energy Gain}

To estimate the intelligence quotient, we're interested in the available energy we gain through our observation of $\dataVariables$, this is denoted 
\begin{align*}
...
\(\dataVariables\) and 
\[
\expDist{\left(D_{\stateVariables|\dataVariables} - \expDist{D_{\stateVariables|\dataVariables}}{\physicsProb(\dataVariables)}\right)}{\trueProb(\dataVariables)}
\]
...
One with the scenario as we've laid it out so far is dealing with the number of variables and/or microstates in the universe. When making a prediction, we don't normally consider all the variables in the universe. There are particular variables we include in our model, and other variables that we choose to ignore. 

In this analysis we describe these two sets of variables as different spaces. We call the space of variables we'd like to ignore the \emph{null space} and we denote them by $\nullVariables$. The variables of interest we call the \emph{domain space} and we denote by $\domainVariables$.

To model in practice, we must make some assumptions about the relationship between the null space and the domain space. In thermodynamic systems, two different assumptions are common. One approach is to consider an 
...
Within the family of physical models that is consistent with our constraint, we can see the model that minimizes the simulation discrimination of the system, \(D_{\stateVariables|\dataVariables}\). We can perform a freeform minimization of the simulation discrimination (see appendix). That tells us that the optimal form of the factor representing the domain variables is given by 
\[
...
The factor representing the null variables can be found and has a symmetric form (see Appendix).
...
Assuming we have made appropriate selections for the null space and the domain space, we can drop the model misspecification, $M_\dataVariables$ term, and we now have an upper bound on the
free energy, 
\[
