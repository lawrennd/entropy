...
The intelligence quotient can be maximized by maximizing the lower bound with respect to $\simProb(\latentVariables)$, which in turn gives us the optimal form of $\physicsProb(\dataVariables)$. The lower bound on the log intelligence quotient is given by,
\[
...
$$
this approximation becomes exact. More generally though the new lower bound is,
$$
\log I \geq -\log \physicsProb(\dataVariables) - \text{KL}\left(\simProb(\domainVariables) || \physicsProb(\domainVariables | \dataVariables)\right)  - \expDist{\log \frac{\physicsProb(\domainVariables|\dataVariables)\simProb(\nullVariables)}{\trueProb(\stateVariables|\dataVariables)}}{\simProb(\domainVariables)\simProb(\nullVariables)}  -\beta E(\measuredVariables_\dataVariables=\textbf{1})
$$
where $\physicsProb(\domainVariables | \dataVariables) = \frac{\exp\left(-\beta \expDist{E(\dataVariables | \stateVariables) + E(\stateVariables)}{\simProb(\nullVariables)}\right)}{Z^\prime_{\domainVariables|\dataVariables}}$.
