...
\begin{equation}
\expDist{\log I}{r(\dataVariables)}   \geq - \beta E(\measuredVariables_\dataVariables=\textbf{1}) - \expDist{\log r(\dataVariables)}{r(\dataVariables)}, \label{eqn-lir-lower}
\end{equation}
...
\]
Maximizing this lower bound is achieved through minimizing $\expDist{\text{KL}(r(\stateVariables)||\trueProb(\stateVariables|\dataVariables))}{\trueProb(\dataVariables)}$, in other words, finding a variational distribution which represents how the microstates behave correctly. Maximising this bound improves the physical plausibility of the model. We call $r(\stateVariables)$ the \emph{simulation} and we call $\exp{ \expDist{\text{KL}(r(\stateVariables)||\trueProb(\stateVariables|\dataVariables))}{\trueProb(\dataVariables)}}$ the simulation fidelity, $F$,
\[
\log F = -\expDist{\text{KL}(r(\stateVariables)||\trueProb(\stateVariables|\dataVariables))}{\trueProb(\dataVariables)}
\]
This in turn gives us an upper bound on our log intelligence ratio,
\[
\log I \leq -\beta E(\measuredVariables_\dataVariables = \textbf{1}) -\log \physicsProb(\dataVariables) - \log F.
\]
and if we consider the expected log intelligence ratio,
\[
\expDist{\log I}{\physicsProb(\dataVariables)} \leq -\beta E(\measuredVariables_\dataVariables = \textbf{1}) -\expDist{\log \physicsProb(\dataVariables)}{\physicsProb(\dataVariables)} - \log F.
\]
then we can see the gap between our lower bound from \ref{eqn-lir-lower} is precisely given by the negative logarithm of the simulation fidelity, $\log F$. If we maximize fidelity, then we can obtain a tight approximation to the intelligence ratio.
...
\end{align*}
\todo{Prove that the first two terms are simply $- \log p(Y)$}
...
\] 
\todo{Also show that this is equal to p{Y}}
