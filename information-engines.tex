...
Maxwell included the demon in his book which gave his theory of heat\citep{Maxwell-theory71}. He was illustrating that the second law holds only in expectation, but the thought experiment intrigued the community and was developed further by Szilard\citep{Szilard-intelligenter29}. Szilard reduced the system to one containing a single particle (now known as a Szilard engine), and he showed that the demon introduces information to the system which counteracts the reduction in entropy. In this way Szilard introduced an equivalence between thermodynamic entropy and information (through the thermodynamic temperature).

The second law of thermodynamics states that entropy cannot decrease as time proceeds in a forward direction. It is through the second law that we understand the impossibility of perpetual motion machines and the irreversibility of certain physical processes. So it is through Maxwell's demon and Szilard's equation of information and energy that we turn to a thermodynamic inspiration for our definition of intelligence. For this reason we use the term Szilard intelligence to refer to our definition.

In the latter half of the 20th century and the first part of this century, discussion about Maxwell's demon centred on how the demon is required to consume energy in the information gathering process . Our focus is different, we are assuming that interactions with the system will require energy (whether through measurements or interventions). So the available energy gain the physical plausibility of intervening in the system in an energy-free manner, the focus firmly being on how the demon Our interventions 
...
\]
where $\beta$ is again the thermodynamic temperature \(\beta = \frac{1}{Tk_B}\). Our definition allows us to express the available energy gain in terms of the logarithm of the quotient as follows,
\[
\Delta A_\dataVariables = T k_B\log I,
\]
so we see that the intelligence quotient is a critical quantity in estimating available energy.

Note that our definition of Szilard intelligence doesn't require a mechanism by which the intelligent action is occurring, it merely relates the measurable and manipulatable states $\dataVariables$, to the microstates of the system, $\stateVariables$, from which available energy originates, through the Hamiltonian.
